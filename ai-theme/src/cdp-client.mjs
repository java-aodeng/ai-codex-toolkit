...
const LOOPBACK_HTTP_HOSTS = ["127.0.0.1", "[::1]"];
const LOOPBACK_WEBSOCKET_HOSTS = new Set(LOOPBACK_HTTP_HOSTS);
...
function parseLoopbackWebSocketUrl(value) {
  ...
  if (
    parsed.protocol !== "ws:" ||
    !LOOPBACK_WEBSOCKET_HOSTS.has(parsed.hostname) ||
    parsed.username ||
    parsed.password ||
    parsed.hash ||
    !parsed.port
    ...
    throw new TypeError(
      "webSocketDebuggerUrl must use an IPv4 or IPv6 loopback address with an explicit port",
    );
  }
...
async function fetchRendererTargetsFromEndpoint(
  endpoint,
...
export async function fetchRendererTargets(
  ...
  const failures = [];
  for (const [index, host] of LOOPBACK_HTTP_HOSTS.entries()) {
    const endpoint = `http://${host}:${port}/json/list`;
    const hostsRemaining = LOOPBACK_HTTP_HOSTS.length - index;
    const remainingMs = Math.max(1, deadline - Date.now());
    const endpointTimeoutMs = Math.max(1, Math.floor(remainingMs / hostsRemaining));
    try {
      return await fetchRendererTargetsFromEndpoint(endpoint, {
        fetchImpl,
        timeoutMs: endpointTimeoutMs,
        deadline: Date.now() + endpointTimeoutMs,
      });
    } catch (error) {
  ...
  const timeoutPrefix = failures.some((error) => /timed out after \d+ms/i.test(errorMessage(error)))
    ? `renderer target discovery timed out after ${timeoutMs}ms: `
    : "";
  throw new Error(
    `${timeoutPrefix}failed to fetch renderer targets from loopback addresses: ${failures.map(errorMessage).join("; ")}`,
    { cause: lastError },
  );
}
...
export async function waitForRendererTargets(
  ...
  while (true) {
    ...
    if (elapsedMs >= timeoutMs || Date.now() >= deadline) {
      throw new Error(
        `timed out after ${timeoutMs}ms waiting for renderer targets on loopback port ${port}: ${lastError.message}`,
        { cause: lastError },
      );
    }
...
export class CdpSession {
  constructor(
    ...
    if (
      ...
      throw new TypeError("domains must be an array of non-empty CDP domain names");
    }
